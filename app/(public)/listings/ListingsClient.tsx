...
// Server-fetched first page of results (default view only). Lets the page
// render real listings in the initial HTML — crawlers see content, users see
// no skeleton flash — while all filtering stays client-side as before.
interface ListingsClientProps {
  initialListings?: ListingData[] | null;
...
function ListingsPageContent({
  ...
  // Server data is only fetched for the default view (no query params). When
  // present and the URL really is the default view, seed state from it and
  // skip the first client fetch — the data is already on screen.
  const useServerData =
    initialListings != null &&
    initialPagination != null &&
    searchParams.toString() === "";
  const skipFirstFetch = useRef(useServerData);

  const [listings, setListings] = useState<ListingData[]>(
    useServerData ? initialListings : []
  );
  ...
  const [loading, setLoading] = useState(!useServerData);
  ...
  useEffect(() => {
    // First run with server-seeded default data: nothing to fetch.
