...
export async function POST(
  ...
  try {
    ...
    if (clientEmail) {
      ...
      const advisorName = advisor?.displayName || advisor?.name || "Your Advisor";

      try {
        await sendEmail({
          to: clientEmail,
          subject: `${advisorName} would like your review on MercatoList`,
          react: ReviewRequestEmail({
            brokerName: advisorName,
            brokerageName: advisor?.brokerageName || undefined,
            reviewUrl,
          }),
        });
      } catch (emailError) {
